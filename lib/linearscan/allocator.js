...
function Allocator(config) {
  ...
  this.inactive = this.registers.slice().sort(Interval.sort);
  this.unhandled = this.intervals.slice().sort(Interval.sort);
...
Allocator.prototype.allocate = function allocate() {
  while (this.handled.length() !== 0) {
    var current = this.handled.shift();
    if (!current.alive)
      continue;
...
Allocator.prototype.allocateFree = function allocateFree(current) {
  ...
  if (current.end() > maxPos)
    this.pushUnhandled(this.split(current, current.start(), maxPos));
...
Allocator.prototype.allocateBlocked = function allocateBlocked(current, pos) {
  ...
  if (firstUse.pos >= maxPos) {
    ...
    this.pushUnhandled(this.split(current, regUse.pos, regUse.pos));
  ...
  // Spill other
  current.value = this.registerOperands[reg];
  this.splitAndSpill(current, pos);
  ...
  // TODO(indutny): figure out split range
  this.pushUnhandled(this.split(current, intersect, intersect));

  return true;
};

Allocator.prototype.split = function split(interval, from, to) {
  return interval.split(to);
};
