...
function Allocator(config, group) {
  ...
  this.inactive = this.config.registerGroup[this.group].filter(notEmpty);
...
Allocator.prototype.hintedSelect = function hintedSelect(current, list) {
  ...
  // Or even better - coalesce with one of the phi's inputs!
  if (current.node.opcode === 'ssa:phi') {
    var maybeHint = this.getPhiHint(current);
    if (maybeHint !== null)
      hint = maybeHint;
  }
...
Allocator.prototype.getPhiHint = function getPhiHint(current) {
  var hint = null;
  var phi = current.node;
  for (var i = 0; i < phi.inputs.length; i++) {
    var pred = phi.block.predecessors[i];
    var predLast = this.positions[pred.nodes[pred.nodes.length - 1].index];
    var input = this.config.intervals[phi.inputs[i].index];

    input = input.childAt(predLast);
    if (input === null ||
        input.value === null ||
        input.value.kind !== 'register') {
      continue;
    }
    hint = input.value.value;
    break;
  }
  return hint;
};
