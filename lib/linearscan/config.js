...
function Config(input, options) {
  ...
  if (this.options.opcodes)
    for (var i = 0; i < this.options.opcodes.length; i++)
      this.defineOpcode(this.options.opcodes[i]);
}
...
Config.prototype.defineOpcode = function defineOpcode(options) {
  var opcode = new Opcode(options.name);
