...
var DynamicOperand = linearscan.DynamicOperand;
...
Config.prototype.getRegisterByName = function getRegisterByName(name) {
  return this.registerMap[name];
};

Config.prototype.getRegisterNameByGroup = function getRegisterNameByGroup(
    group, index) {
  return this.options.registers[group][index];
};

Config.prototype.getRegisterName = function getRegisterName(index) {
  return this.registerName[index];
};
...
Config.prototype.renderGapMove = function renderGapMove(move, pipeline) {
  ...
  if (move.src >= 0)
    src = pipeline.reg(this.getRegisterName(move.src));
  else
    src = pipeline.spill(-1 - move.src);

  if (move.dst >= 0)
    dst = pipeline.reg(this.getRegisterName(move.dst));
  else
