...
Config.prototype.createOpcode = function createOpcode(name, options) {
  ...
  if (options.scratches) {
    for (var i = 0; i < options.scratches.length; i++) {
      var scratch = this.createOperand(options.scratches[i]);
      assert.equal(scratch.kind, 'register', 'Non-register scratch requested');
      assert(scratch.value !== null, 'Any register scratch requested');

      opcode.scratches.push(scratch);
    }
  }
