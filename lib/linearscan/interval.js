...
function Interval(node, opcode, group) {
  this.node = node;
  this.opcode = opcode;
  this.group = group;
  this.ranges = [];
...
Interval.prototype.use = function use(pos, value) {
  ...
  // Merge uses
  if (index < list.length && list[index].pos === pos) {
    if (!list[index].merge(use))
      return false;
...
Interval.prototype._firstUseAfter = function _firstUseAfter(uses, pos, kind) {
  var index = binarySearch(uses, pos, Use.needleSort);
  if (index < 0)
    index = -1 - index;
...
Interval.prototype.firstUseAfter = function firstUseAfter(pos, kind) {
  return this._firstUseAfter(this.uses, pos, kind);
};

Interval.prototype.firstFixedUseAfter = function firstFixedUseAfter(pos) {
  return this._firstUseAfter(this.fixedUses, pos);
};
...
Interval.prototype.split = function split(pos) {
  var child = new Interval(this.node, this.opcode, this.group);
...
Interval.prototype.intersect = function intersect(other) {
  ...
  while (i < this.ranges.length && j < other.ranges.length) {
    ...
    var check = b.intersect(a);
    if (check !== false)
      return check;

    while (i < this.ranges.length && this.ranges[i].end <= b.start)
      i++;
