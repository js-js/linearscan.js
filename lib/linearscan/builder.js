...
Builder.prototype.buildNodeIntervals = function buildNodeIntervals(
  ...
  if (opcode.output === null)
    ...
    out.use(pos, opcode.output.resolve(this.config, node));
  ...
  for (var i = 0; i < node.inputs.length; i++) {
    ...
    interval.use(pos, opcode.inputs[i].resolve(this.config, node));
    live.add(input.index, input);
  }

  for (var i = 0; i < opcode.spills.length; i++) {
    var spill = opcode.spills[i].resolve(this.config, node);
