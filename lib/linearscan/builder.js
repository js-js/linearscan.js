...
Builder.prototype.splitFixed = function splitFixed() {
  for (var i = this.intervals.length - 1; i >= 0; i--) {
    ...
    for (var j = interval.fixedUses.length - 1; j >= 0; j--) {
      ...
      this.splitFixedWithUse(interval, start, end, use);
    }
  }
};
...
Builder.prototype.splitFixedWithUse = function splitFixedWithUse(interval,
  ...
  // Out-of-bands use - will be covered by move resolver
  if (use.pos >= end || use.pos < start)
    return;

  // TODO(indutny): register hints
  // Split after use
  var fixed;
  if (use.pos === end) {
    // Split right before the end
    if (use.pos !== start)
      fixed = interval.split(use.pos - 1);
  } else {
    // Divide into three: head, middle, and tail
    if (use.pos + 1 !== end)
      this.intervals.push(interval.split(use.pos + 1));
