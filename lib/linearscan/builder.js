...
Builder.prototype.inputPos = function inputPos(block, index) {
  // Extra padding at the start and at the end of the block
  return 1 + block.blockIndex * 2 + index;
};

Builder.prototype.spillPos = function spillPos(block, index) {
  return this.inputPos(block, index);
};

Builder.prototype.outputPos = function outputPos(block, index) {
  return this.inputPos(block, index);
};

Builder.prototype.initBlockRanges = function initBlockRanges() {
  var offset = this.input.blocks.length;
  var start = this.inputPos(this.input.blocks[0], offset);
  for (var i = 1; i < this.input.blocks.length; i++) {
    ...
    var end = this.inputPos(block, block.nodes[0].index);
    this.intervals[i - 1].fillRange(start - 1, end - 1);
    start = end;
  }

  var last = this.input.blocks[i - 1];
  var end = this.inputPos(last, last.nodes[last.nodes.length - 1].index) + 2;
  this.intervals[i - 1].fillRange(start - 1, end - 1);
...
Builder.prototype.buildNodeIntervals = function buildNodeIntervals(
  ...
  var inputPos = this.inputPos(block, node.index);
  var outPos = this.outputPos(block, node.index);
  ...
  var spillPos = this.spillPos(block, node.index);
  for (var i = 0; i < opcode.spills.length; i++) {
    var spill = opcode.spills[i];
    this.registers[spill.value].fillRange(spillPos, spillPos + 1);
  }
};
...
Builder.prototype.killPhis = function killPhis(loopStart, live, block) {
  ...
  for (var i = 0; i < block.nodes.length; i++) {
    ...
    var out = this.intervals[node.index];
    out.use(this.outputPos(block, node.index), opcode.output);
    live.remove(node.index);
...
Builder.prototype.splitFixedWithUse = function splitFixedWithUse(interval,
  ...
  if (use.pos !== interval.end() && use.pos !== interval.start()) {
