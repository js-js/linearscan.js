...
Linearscan.prototype.init = function init() {
  ...
  Object.keys(this.options.instructions).forEach(function(type) {
    ...
    this.declarations[type] = {
      output: instr.output === null ? null : (instr.output || { type: 'any' }),
      inputs: instr.inputs || [],
      scratch: instr.scratch || [],
      shallow: !!instr.shallow,
      tainted: !!instr.tainted,
      call: !!instr.call
    };
  }, this);
};
...
Linearscan.prototype.prerun = function preprun() {
  ...
  this.registerList = this.registers.map(function(reg) {
    var interval = this.createInterval();
    interval.fix({ type: 'register', id: reg });
    this.registerMap[reg] = interval;

    return interval;
  }, this);
  this.taintKiller = this.createInterval();
  this.taintKiller.fix({ type: 'killer' });
};
...
Linearscan.prototype.clone = function clone(blocks) {
  ...
  blocks = blocks.map(function(block, i) {
    var res = new Block(this, block, instructionMap, i);
    if (block.id)
...
Linearscan.prototype.reorder = function reorder() {
  ...
  function blockCompare(a, b) {
    return a.loopIndex === b.loopIndex ? a.originalIndex - b.originalIndex :
...
Linearscan.prototype.enumerate = function enumerate() {
  ...
  for (var i = 0; i < blocks.length; i++) {
    ...
    var instructions = [ ];
    pos = this.gap(block, instructions, pos);
    for (var j = 0; j < block.instructions.length; j++) {
      ...
      if (instr.hasCall || instr.overwritesInput)
        pos = this.gap(block, instructions, pos);
      pos = this.gap(block, instructions, pos);
    }
    // End block with gap
...
Linearscan.prototype.gap = function gap(block, instructions, pos) {
        ...
        if (input instanceof Instruction) {
          assert(input.output,
                 'Instruction ' + input.type +
                     ' has no output, but is used at ' + instr.id);
          if (!block.liveKill[input.output.id])
  ...
  for (var i = blocks.length - 1; i >= 0; i--) {
    ...
    for (var j = block.instructions.length - 1; j >= 0; j--) {
      ...
      instr.inputs.forEach(function(input, i) {
        if (input instanceof Instruction) {
          var end = decl.shallow ? instr.pos : instr.pos + 1;
          if (!input.output.covers(end))
            input.output.addRange(block.start, end);
          assert(decl.inputs.length > i,
                 'Not enough inputs in declaration of ' + instr.type);
...
Linearscan.prototype.splitFixed = function splitFixed() {
  Object.keys(this.intervals).forEach(function(id) {
    ...
    for (var i = 0; i < uses.length - 1; i++) {
      ...
      if (prev.kind.id || next.kind.id)
        interval = this.splitBetween(interval, prev.pos, next.pos - 1);
    }
  }, this);
};
...
function unhandledSort(a, b) {
  ...
  var useA = a.firstFixedUse('register', 0);
  var useB = b.firstFixedUse('register', 0);
...
Linearscan.prototype.allocateFree = function allocateFree(current) {
  ...
  if (use !== null) {
    ...
    if (!this.instructions[use.pos].hasCall &&
        (use.pos === current.start() || use.pos === maxPos)) {
      maxPos = use.pos + 1;
    }
  } else {
    for (var i = 0; i < this.registers.length; i++) {
...
Linearscan.prototype.allocateBlocked = function allocateBlocked(current) {
  ...
  if (firstUse === null || maxPos < firstUse.pos) {
    ...
    if (hint !== null && hint.type === 'stack')
      current.value = this.getSpill(hint);
    else
      current.value = this.getSpill();
    this.activeSpills.push(current);
    if (firstUse !== null)
      this.splitBetween(current, current.start(), firstUse.pos - 1);
  } else {
    assert(use !== null || blockPos[id] !== 0,
...
Linearscan.prototype.getSpill = function getSpill(hint) {
  var spill;

  if (hint) {
    var index = null;
    this.spills.some(function(spill, i) {
      if (spill.id !== hint.id)
        return false;
...
Linearscan.prototype.splitAndSpill = function splitAndSpill(interval) {
  ...
  for (var i = 0; i < queue.length; i++) {
    ...
    var use = child.firstUse('register', splitStart);
    // If child has a register value right after the split - push it to
    // unhandled instead
    if (use !== null && use.pos <= child.start() + 1) {
      assert(interval.start() <= child.start(), 'Progress should be made');
...
Linearscan.prototype.mergeToPhis = function mergeToPhis() {
...
Linearscan.prototype.mergeGaps = function mergeGaps() {
  this.blocks.forEach(function(block) {
    block.mergeGaps();
  });
};
...
Linearscan.prototype.splitBetween = function splitBetween(interval,
  ...
  if (!gap.hasGap)
    tryPrev = true;
  if (splitPos > 0 && this.instructions[splitPos - 1].overwritesInput)
    tryPrev = true;
  if (splitPos === interval.end())
    tryPrev = true;
  if (tryPrev) {
    splitPos--;
...
Linearscan.prototype.cleanTainted = function cleanTainted() {
  Object.keys(this.intervals).forEach(function(id) {
    var interval = this.intervals[id];
    if (!interval.tainted || interval.parent !== null)
      return;

    assert.equal(interval.value.type, 'register', 'Taint interval not reg');

    var list = [];
    interval.uses.forEach(function(use) {
      list.push({ use: use, interval: interval });
    }, this);

    interval.children.forEach(function(child) {
      assert.equal(child.value.type, 'register', 'Taint interval not reg');
      child.uses.forEach(function(use) {
        list.push({ use: use, interval: child });
      }, this);
    }, this);

    // Remove uses from the same block
    for (var i = list.length - 1; i > 0; i--)
      if (list[i].use.instr.block === list[i - 1].use.instr.block)
        list.splice(i - 1, 1);

    // Handle branches, where interval goes only to one branch
    var parent = interval;
    list.forEach(function(item) {
      var use = item.use;
      var interval = item.interval;

      var block = use.instr.block;
      if (!block.liveOut[parent.id])
        return;
      ...
      if (block.successors[0].liveIn[parent.id]) {
        live = block.successors[0];
        kill = block.successors[1];
      } else {
        live = block.successors[1];
        kill = block.successors[0];
      }

      // One should kill, other should use
      if (kill.liveIn[parent.id] || !live.liveIn[parent.id])
        return;

      var gap = kill.instructions[0];
      assert(gap.hasGap, 'No gap at the start of block');

      killInterval.call(this, gap, interval);
    }, this);

    // Filter-out dead ends
    list = list.filter(function(item) {
      return !item.use.instr.block.liveOut[parent.id];
    });
    ...
    list.forEach(function(item) {
      ...
      var gap = this.instructions[use.instr.pos + 1];
      assert(gap.hasGap, 'Taint killer not on gap');

      // Handle overwrite
      var out = use.instr.output;
      if (out !== null &&
          out.value.type === 'register' &&
          out.value.id === interval.value.id) {
        return;
      }
  ...
  function killInterval(gap, interval) {
    // Handle move overwrite
    var overwrite = gap.pendingMoves.some(function(move) {
      return move.to.value.type === 'register' &&
             move.to.value.id === interval.value.id;
    });
    if (overwrite)
      return;

    gap.pendingMoves.push({
      from: this.taintKiller,
      to: interval,
      resolved: true
    });
  }
...
function Block(ls, block, instructions, index) {
  this.ls = ls;
  this.id = block.id;
  this.uid = ls.blockId++;
...
Block.prototype.mergeToPhis = function mergeToPhis() {
  for (var i = this.instructions.length - 1; i >= 1; i--) {
    var instr = this.instructions[i];
    var prev = this.instructions[i - 1];

    if (instr.type !== 'to_phi' || !prev.hasGap)
      continue;

    var from = instr.inputs[0].output.childAt(instr.pos);
    var to = instr.output.childAt(instr.pos);

    prev.pendingMoves.push({
      from: from,
      to: to,
      resolved: true
    });
...
Block.prototype.mergeGaps = function mergeGaps() {
  for (var i = this.instructions.length - 1; i >= 1; i--) {
    var instr = this.instructions[i];
    var prev = this.instructions[i - 1];

    if (instr.type !== 'gap' || prev.type !== 'gap')
      continue;

    this.instructions.splice(i, 1);
    instr.removed = true;

    prev.mergeMoves(instr.moves);
  }
};
...
function Instruction(ls, block, instr) {
  ...
  this.astId = instr.astId || null;
  this.ast = instr.ast || null;
  ...
  this.overwritesInput = this.decl.output && this.decl.output.id &&
      this.decl.inputs.some(function sameAsRegOut(input) {
        return input.type ==='register' &&
               input.type === this.decl.output.type &&
               input.id === this.decl.output.id;
      }, this);

  var hasGap = this.type === 'gap' || this.hasCall || this.overwritesInput;
  this.hasGap = hasGap;
  ...
  if (this.decl.tainted)
    this.output.tainted = true;
...
Instruction.prototype.mergeMoves = function mergeMoves(moves) {
  ...
  for (var i = 0; i < moves.length; i++) {
    ...
    if (merge.type === 'move') {
      for (var j = 0; j < len; j++) {
        ...
        // a => b + b => c = a => c
        if (move.to.type === merge.from.type && move.to.id === merge.from.id) {
          move.to = merge.to;
          merge = null;
          break;
        }
...
Interval.prototype.addUse = function addUse(pos, instr, kind) {
  if (this.tainted) {
    assert.equal(kind.type, 'register',
                 'Tainted interval could have only register uses');
  }

  if (kind.type === 'register' && kind.id) {
    var fixed = this.ls.registerMap[kind.id];
    if (!fixed.covers(pos))
      fixed.addRange(pos, pos + 1);
  }
...
Interval.prototype.hint = function hint() {
  ...
  if (hints.length > 0) {
    ...
    // First fixed child
    interval = this.children[0];
  }

  if (!interval)
    return null;

  var use = interval.firstFixedUse('register', 0);
  if (use !== null)
    return use.kind;
