...
Linearscan.prototype.splitFixed = function splitFixed() {
  Object.keys(this.intervals).forEach(function(id) {
    ...
    var sid = interval.id;
    for (var i = 0; i < interval.uses.length; i++) {
      var use = interval.uses[i];

      if (use.kind.type !== 'register' || !use.kind.id)
        continue;

      // Interval starts with a fixed use - skip it
      if (interval.start() >= use.instr.pos - 1)
        continue;

      var pos = use.instr.pos - 1;
      interval = this.splitBetween(interval, interval.start(), pos);
      interval.value = { type: 'register', id: use.kind.id };
...
Linearscan.prototype.walkIntervals = function walkIntervals() {
  ...
  while (this.unhandled.length !== 0) {
    ...
    } else if (current.value.type === 'register') {
      // Fixed value, split and spill, if blocked
...
Linearscan.prototype.allocateFree = function allocateFree(current) {
  ...
  for (var i = 0; i < this.registers.length; i++) {
    ...
    if (freePos[reg] < maxPos ||
        freePos[reg] === maxPos && (hint === null || hint !== reg)) {
      continue;
    }
    maxPos = freePos[reg];
    id = reg;
...
Linearscan.prototype.allocateBlocked = function allocateBlocked(current) {
  ...
  for (var i = 0; i < this.registers.length; i++) {
    ...
    if (usePos[reg] < maxPos ||
        usePos[reg] === maxPos && (hint === null || hint !== reg)) {
      continue;
    }
    maxPos = usePos[reg];
    id = reg;
  ...
  for (var i = 0; i < this.blocks.length; i++) {
    ...
    for (var j = 0; j < block.successors.length; j++) {
      ...
      Object.keys(succ.liveIn).forEach(function(id) {
        ...
        if (from !== to)
          gap.pendingMoves.push({ from: from, to: to });
      }, this);
    }
  }
};
...
Linearscan.prototype.splitBetween = function splitBetween(interval, from, to) {
  ...
  if (!gap.hasGap)
    gap = this.instructions[splitPos - 1];
  if (!boundary)
...
Instruction.prototype.init = function init(instructions) {
  ...
  if (this.phi !== null) {
    this.output.hintIntervals.push(this.inputs[0].output);
  }
};
...
Instruction.prototype.resolveMoves = function resolveMoves() {
  ...
  for (var i = 0; i < moves.length; i++)
    if (status[i] === 'to_move')
      this.resolveOne(moves, status, i, this.moves);
};
...
Instruction.prototype.resolveOne = function resolveOne(moves, status, i, res) {
  ...
  for (var j = 0; j < moves.length; j++) {
    ...
    if (nextFrom.type !== to.type || nextFrom.id !== to.id)
      continue;
...
Interval.prototype.split = function split(pos, child) {
  ...
  for (var i = 0; i < this.uses.length; i++) {
    ...
    if (usePos > pos)
      break;
    if (this.ls.instructions[pos].hasCall && usePos === pos) {
      i++;
      break;
    }
  }
...
Interval.prototype.hint = function hint() {
  var hints = this.hintIntervals.filter(function(interval) {
    return interval.value.type === 'register';
  });
  ...
  if (!interval)
    return null;
  return interval.value.type === 'register' ? interval.value.id : null;
};
