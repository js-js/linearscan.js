...
Linearscan.prototype.splitToPhis = function splitToPhis() {
  Object.keys(this.intervals).forEach(function(id) {
    ...
    for (var i = 0; i < interval.phiHints.length; i++) {
      ...
      debugger;
      this.splitBetween(child, hint.instr.pos, hint.instr.pos);
    }
  }, this);
};
...
Linearscan.prototype.splitBetween = function splitBetween(interval, from, to) {
  ...
  assert(from < splitPos && splitPos <= to || from === to, 'Split OOB');
  ...
  if (!boundary && this.instructions[splitPos].type !== 'to_phi')
